...
func (w *Wallet) CreateRawTransaction(to string, amount int64) (string, error) {
	publicKeyECDSA := w.wallet.DeriveNativePublicKey()
	...
	value := big.NewInt(amount * 1000000000) // in wei (1 eth)
	...
	signedTx, err := types.SignTx(tx, types.NewEIP155Signer(chainID), w.wallet.DeriveNativePrivateKey())
	if err != nil {
		return "", err
	}
...
func CreateERC20Transaction() {
	...
	addrA0 := common.HexToAddress(a0)
	addrA1 := common.HexToAddress(a1)

	client, killGanache, err := RunGanache(w0.DerivePrivateKey()) // w0 init 100 ehter
	if err != nil {
		log.Fatal(err)
	}
	defer killGanache()
	...
	{ // deploy contract
		...
		err = baseParam.EnsureGasPrice(rpcClient)
		opts, err := MakeTransactOpts(w0, baseParam, -1, -1)
		if err != nil {
			log.Fatal(err)
		}

		var tx *types.Transaction
		contractAddr, tx, err = eth.DeployContract(opts, rpcClient, tetherAbi, tetherBytecode,
		...
		fmt.Printf("depoly contract, gas limit: %d, gas used: %d\n, address: %s\n", tx.Gas(), receipt.GasUsed, receipt.ContractAddress.String())
	}

	tokenAmount := big.NewInt(1000000)
	contract := eth.NewErc20Contract(contractAddr, rpcClient)
	...
	{ // transfer erc20
		...
		receipt, err := rpcClient.TransactionReceipt(context.Background(), tx.Hash())
		fmt.Printf("transfer token, gas limit: %d, gas used: %d\n", tx.Gas(), receipt.GasUsed)
	}
}
...
func RunGanache(accountPrivateKeys ...string) (*rpc.Client, func(), error) {
	...
	if len(accountPrivateKeys) > 0 {
		for _, a := range accountPrivateKeys {
			args = append(args, fmt.Sprintf("--account=0x%s,100000000000000000000 --chainId 3", a)) //100 Ether
		}
	}
	...
	cli, err := rpc.Dial(fmt.Sprintf("http://127.0.0.1:%d", ganacheCliPort))
	if err != nil {
		killFunc()
		return nil, nil, err
	}
