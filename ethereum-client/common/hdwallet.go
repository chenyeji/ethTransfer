...
func (w *Wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
  ...
  // Sign the transaction and verify the sender to avoid hardware fault surprises
	...
	fmt.Println("签名前messageHash:"+ hexutil.Encode(messageHash.Bytes()))


	spew.Dump(signedTx.Hash())
	sender, err := types.Sender(signer, signedTx)
	if err != nil {
		return nil, err
	}
	...
	//rawdata, _ := rlp.EncodeToBytes(signedTx)
	fmt.Println("待广播String："+ hexutil.Encode(data))
	//fmt.Println("签名前rlpString："+ hex.EncodeToString(rawdata))
	fmt.Println("交易Hash："+ signedTx.Hash().String())
	if sender != account.Address {
		return nil, fmt.Errorf("signer mismatch: expected %s, got %s", account.Address.Hex(), sender.Hex())
	}
